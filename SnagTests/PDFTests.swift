...
final class PDFTests: XCTestCase {
    func testOnePagePerRoomAndTheLastPageSaysWhatTheReportIs() throws {
        let report = Report(kind: .moveIn, address: "14 Admiralty Way", createdAt: 1_789_000_000, rooms: [
            ...
            Room(name: .bedroom, width: Extent(centimetres: 320, tier: .measured), length: Extent(centimetres: 410, tier: .measured), area: Extent(centimetres: 131_200, tier: .measured)),
        ])
        ...
        XCTAssertTrue(text[2].contains("Bedroom") && text[2].contains("measured") && text[2].contains("3.20") && text[2].contains("4.10") && text[2].contains("13.1"), "the tier is beside the number — \(text[2])")
